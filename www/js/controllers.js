...
.controller('ScatterPlotCtrl', function ScatterPlotCtrl($scope, database, chemo) {
    ...
    database.inputDataFile(absorb, conc, lables, wave, "test");
    output = database.outputDataFile("test");
    /*var absorb = [1, -3, 2, 6, 8, 3, -2];
    var conc = [1, 1, 1, 0, -1];
    var lables = ["a", "b", "c", "d", "e"];
    var wave = [2, 4, 6, 8, 10, 12, 14];*/
    database.inputDataFile(absorb, conc, lables, wave, "test2");
    output = database.outputDataFile("test2");
    var result = chemo.train(false, ['test', 'test2']);
    console.log(result);
    output = chemo.getPCA();
    console.log(output);
